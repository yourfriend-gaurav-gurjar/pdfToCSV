...
# Assign bucket names.
source_bucket = 'cannaeyeglass-datalake-processed-qa'
dest_bucket = 'cannaeyeglass-datalake-enriched-qa'
...
def csvToCombinedcsv(dest_bucket, destination_file, source_bucket, file_path):
    ...
    enrich_df= raw_df.iloc[:,:8]
    enrich_df.columns=['Name','Lincence_number','Email','Phone','City','Zip','County','license_type']
...
processed_bucket = 'cannaeyeglass-datalake-processed-qa'
enriched_bucket = 'cannaeyeglass-datalake-enriched-qa'
...
def enrichment(enriched_bucket, destination_file, processed_bucket, file_path, license_type):
    ...
    # processed_df = processed_df.drop(['Unnamed: 0', 'Unnamed: 0.1'])
    ...
    #     missing_data.drop('2', inplace=True, axis=1)
        processed_df.columns = ['business', 'license', 'email', 'phone', 'city', 'zip', 'county', 'license_type']
    #     missing_data.columns = ['business', 'license', 'email', 'phone', 'city', 'zip', 'county', 'license_type']
        processed_df = processed_df.replace('\n', '', regex=True)
        processed_df['zip'] = processed_df['zip'].astype("int64")
        data = processed_df['business'].str.split(r'Trade Name:', expand=True)
        processed_df['trade_name'] = data[1]
        processed_df['business'] = data[0]
        # print(processed_df['license_type'])
...
for file in filenames:
    ...
    combained='US/OK/LicenseInfo/' + year + '/' + month + '/' + day + '/' + file+'_' + date+'.csv'
    csvToCombinedcsv(dest_bucket, combained, source_bucket, raw_path_dir,file)
    # enrich_path_dir = 'US/OK/LicenseInfo/'  + year + '/' + month + '/' + day
    # enrich_path_dir = 'US/OK/LicenseInfo/' + year + '/' + month + '/' + day
